...
describe('the routing call', () => {
  ...
  it('asks for a handful of tokens, not a thousand', async () => {
    await run();
    // One enum value under a strict schema; a large budget only lets a reasoning
    // model stall here before emitting a single word.
    expect(calls[0].maxOutputTokens).toBeLessThanOrEqual(16);
  });
