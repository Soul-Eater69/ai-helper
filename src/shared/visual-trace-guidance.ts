...
The primary display is a copyable notebook for a plain shared editor or virtual whiteboard. Make the trace understandable without colors, animation, or clicking Next. Start with the exact example and a small diagram, then develop it through concrete actions and state updates.
Each write field is the actual concise notes the candidate can type directly, NOT instructions like "update the queue". Include the operation, before -> after changes, and resulting queue/stack/map/order/counters needed to follow that step. For example: "take 0\nindegree[1]: 1 -> 0\nindegree[2]: 1 -> 0\nqueue = [1, 2]\norder = [0]". Keep initial values and variable labels aligned and clear. Include edge meaning in the setup, such as "prerequisite -> course". Show one meaningful decision per step and finish with the actual return value. The say field is one or two simple spoken sentences explaining WHY that update is valid: "Now 0 is done, so 1 and 2 each lose one prerequisite. Both counts reach zero, so I can add them to the queue." Avoid formal phrasing, narration of UI actions, or claims about interview technique.
A dry run must demonstrate the mechanism, not just the easy exit. Trees show calls, saved child results and returns; graphs show the relevant edge, readiness/visited changes and queue; grids show each minute/frontier; DP shows numeric source values, the calculation and stored result. Include the revealing blocked/not-ready case when present. Keep state notes compact enough to reproduce by hand. Do not use a trivial example solely to avoid repeating an earlier trace.
